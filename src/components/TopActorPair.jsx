...
function TopActorPair({ actors = [], roles = [], movies = [] }) {
  ...
  const topPair = useMemo(() => {
    ...
    for (const r of roles) {
      ...
      if (!actorsByMovie[MovieID]) actorsByMovie[MovieID] = new Set();
    ...
    return { actor1, actor2, count: maxCount, sharedMovies };
  }, [actors, roles, movies]);
  ...
  if (actors.length < 2) return <div className="top-actor-pair empty">Add at least two actors to see co-stars.</div>;
  if (roles.length === 0)  return <div className="top-actor-pair empty">Add some roles to find co-star pairs.</div>;
  if (movies.length === 0) return <div className="top-actor-pair empty">Add movies to compute co-stars.</div>;
  if (!topPair)           return <div className="top-actor-pair empty">No top actor pair found yet.</div>;

  const selectedMovieRoles = selectedMovie
    ? roles.filter((r) => String(r.MovieID) === String(selectedMovie.ID))
    : [];
    ...
    <div className="top-actor-pair">
      ...
      {selectedMovie && (
        <MovieModal
          ...
          movie={selectedMovie}
          roles={selectedMovieRoles}
          actors={actors}
          isEditing={false}        // read-only
          editData={{ Title: "", ReleaseDate: "" }} // unused in view mode
          setEditData={() => {}}
          onEditClick={() => {}}
          onSaveEdit={() => {}}
          onCancelEdit={() => {}}
          onDelete={() => {}}
          allMovies={movies}
          hideActions={true}       // hides Edit/Delete buttons
        />
      )}
    </div>
  );
